...
export const topicExplorer = {
    topicPlot: function () {
        ...
        // var viewBox = topicPlotSvg.attr("viewBox");
        ...
        // word cloud setter & getter function
        var wordCloud = null;
        var wordCloud_ = function (_) {
            var that = this;
            if (_ === undefined) return wordCloud;
            console.log("topicPlot.wordCloud() function called.");
            wordCloud = _;
            return that;
        };
            ...
            var topicCircles = topicPlotSvg.selectAll("circle")
                ...
                .on("click", function (d) {
                    ...
                    topicCircles.transition()
                        .duration(transitionDuration)
                        .attr("r", d => radScale(calcRadius(d.size)));
                    wordCloud.changeImage(d.wordcloud);
                    var boundingCircle = d3.select(this);
                    boundingCircle.transition()
                        .duration(transitionDuration)
    ...
    wordCloud: function() {
        ...
        var wordCloudSvg = d3.select("svg#word-cloud")
            ...
            // .attr("width", "50%")
            ;
        // var width = wordCloudSvg.attr("width"),
        //     height = wordCloudSvg.attr("height");
        ...
        var changeImage_ = function (imageBase64String) {
            var that = this;
            wordCloudImg.attr("xlink:href", "data:image/png;base64," + imageBase64String);
            return that;
        };

        var clearImage_ = function () {
            var that = this;
            wordCloudImg.attr("xlink:href", "data:image/png;base64," + defaultImageByteText);
            return that;
        };
        return {
            "changeImage": changeImage_,
    ...
    api: function () {
        ...
        var getTopicsData_ = function () {
            var xhttp = new XMLHttpRequest();
            xhttp.open("GET", getTopicsDataUrl, false);
            xhttp.send();
