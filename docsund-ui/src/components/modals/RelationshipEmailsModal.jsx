...
export default class RelationshipEmails extends React.Component {
  ...
  render() {
    ...
    if (appStore.loadingRelationshipEmails) {
      title = "..."
    } else if (fromNode.labels[0] === "Person") {
      if (toNode.labels[0] === "Person") {
        title = (
          <span>
            Emails between
            {" "}
            <Tag color="purple">{fromNode.propertyMap.email}</Tag>
            and
            {" "}
            <Tag color="purple">{toNode.propertyMap.email}</Tag>
          </span>
        )
      } else {
        title = (
          <span>
            ...
            mentioning
            {" "}
            <Tag color="blue">{toNode.propertyMap.name}</Tag>
          </span>
        )
      }
    } else {
      title = (
        <span>
          Emails mentioning
          {" "}
          <Tag color="blue">{fromNode.propertyMap.name}</Tag>
          and
          {" "}
          <Tag color="blue">{toNode.propertyMap.name}</Tag>
        </span>
